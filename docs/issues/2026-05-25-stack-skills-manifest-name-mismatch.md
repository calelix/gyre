...
## Summary
The stack-skills manifest declares two entries with bare names
...
fail to install with "No matching skills found", while a sibling entry
...
- **Actual:** the CLI returns `No matching skills found for:
  ...
  available names. The host tree and `skills-lock.json` are left unchanged for
  the two failed entries.

## Root cause
The manifest's `name` / `skill` columns for those two entries are written
...
prefix and the manifest does not reflect the per-skill reality.

## Proposed remediation
- [`skills/setup/references/stack-skills.md`](../../skills/setup/references/stack-skills.md)
  — update the two affected rows' `name` and `skill` columns to
  `vercel-react-best-practices` and `vercel-composition-patterns`. No other
  rows need to change. `generate-component`'s precondition (Step 1.5) reads
  `name` from the manifest dynamically, so its detection keys move with the
  change without a separate edit.
- [`docs/superpowers/specs/2026-05-24-stack-skills-precondition-and-manifest-expansion-design.md`](../superpowers/specs/2026-05-24-stack-skills-precondition-and-manifest-expansion-design.md)
  — the design spec carries the same Skills table; update its two affected
  rows to match, so the historical spec does not mislead future readers.
...
will install the two previously failed entries.
